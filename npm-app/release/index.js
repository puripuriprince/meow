...
const CONFIG = {
  ...
  binaryName: process.platform === 'win32' ? 'andji.exe' : 'andji',
  githubRepo: 'andjiAI/andji-community',
  userAgent: 'andji-cli',
...
const PLATFORM_TARGETS = {
  'linux-x64': 'andji-linux-x64.tar.gz',
  'linux-arm64': 'andji-linux-arm64.tar.gz',
  'darwin-x64': 'andji-darwin-x64.tar.gz',
  'darwin-arm64': 'andji-darwin-arm64.tar.gz',
  'win32-x64': 'andji-win32-x64.tar.gz',
}
...
async function downloadBinary(version) {
  ...
  try {
    // Find the extracted binary - it should be named "andji" or "andji.exe"
  ...
  if (isPrintMode) {
    ...
    console.log('Download complete! Starting andji...')
  }
}
    ...
    try {
      ...
      if (isPrintMode) {
        console.error(
          JSON.stringify({
            type: 'error',
            message: `Failed to download andji: ${error.message}`,
          }),
        )
      } else {
        console.error('❌ Failed to download andji:', error.message)
        console.error('Please check your internet connection and try again')
      }
      process.exit(1)
    }
...
async function main(firstRun = false, printMode = false) {
  ...
  try {
    ...
    if (firstRun) {
      if (!isPrintMode) {
        console.error('❌ andji failed to start:', error.message)
        console.log('Redownloading andji...')
      }
