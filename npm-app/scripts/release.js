...
async function triggerWorkflow(versionType) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/andjiAI/andji/actions/workflows/npm-app-release-prod.yml/dispatches \
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/andjiAI/andji/actions/workflows/npm-app-release-prod.yml',
      )
    } else {
      // log(
    ...
    log(
      'You may need to trigger it manually at: https://github.com/andjiAI/andji/actions/workflows/npm-app-release-prod.yml',
    )
  }
}
...
async function main() {
  ...
  log('')
  log('Monitor progress at: https://github.com/andjiAI/andji/actions')
}
