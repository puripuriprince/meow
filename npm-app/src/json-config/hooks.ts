...
import { loadandjiConfig } from './parser'
...
/**
 * Runs file change hooks defined in the andji.json configuration.
...
export async function runFileChangeHooks(filesChanged: string[]): Promise<{
  toolResults: andjiToolOutput<'run_file_change_hooks'>
  someHooksFailed: boolean
}> {
  const config = loadandjiConfig()
  const toolResults: andjiToolOutput<'run_file_change_hooks'> = [
    { type: 'json', value: [] },
  ]
