...
import type { JSONObject } from '@andji/common/types/json'
...
export function cleanupStoredProcesses(): {
  separateandjiInstanceRunning: boolean
  cleanUpPromise: Promise<any>
} {
  // Determine which processes to kill (sync)
  let separateandjiInstanceRunning = false
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (shouldKillProcessUsingLock(lockFile)) {
        ...
        separateandjiInstanceRunning = true
      }
    }
  } catch {}

  if (locksToProcess.length) {
    console.log(gray('Detected running andji processes. Cleaning...\n'))
    logger.info({
