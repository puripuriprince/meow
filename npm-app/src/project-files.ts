...
import { getFileTokenScores } from '@andji/code-map'
import {
  andjiConfigFile,
...
import { ensureDirectoryExists } from '@andji/common/util/file'
import { filterObject } from '@andji/common/util/object'
  ...
  if (existsSync(dir)) {
    if (projectRoot && projectRoot !== dir) {
      ...
      console.log(
        ...
        `andji will read and write files in "${dir}".\n`,
      )
    }
    projectRoot = dir
    if (setCwd) {
  ...
  if (
    ...
    const knowledgeFilePaths = allFilePaths.filter((filePath) => {
      ...
      return (
        ...
        lowercaseFilePath === andjiConfigFile.toLowerCase() ||
        lowercaseFilePath === andjiConfigFileBackup.toLowerCase()
      )
    })
