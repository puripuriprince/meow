...
import { FileChangeSchema } from '@andji/common/actions'
import { BrowserActionSchema } from '@andji/common/browser-actions'
import { SHOULD_ASK_CONFIG } from '@andji/common/old-constants'
import { truncateStringWithMessage } from '@andji/common/util/string'
...
import type { BrowserResponse } from '@andji/common/browser-actions'
import type {
  ClientToolCall,
  ClientToolName,
  andjiToolOutput,
...
import type { ToolCall } from '@andji/common/types/session-state'
    ...
    result.push([
      {
        ...
        value: {
          file,
          errorMessage:
            'Failed to write to file: file is ignored by .gitignore or .andjiignore',
        },
      },
    ])
  }
...
const handleBrowserLogs: ToolHandler<'browser_logs'> = async (params, _id) => {
  ...
  try {
    ...
    ] satisfies andjiToolOutput<'browser_logs'>
  } finally {
    Spinner.get().stop()
  }
