    ...
    for (const submodule of submodules) {
      try {
        ...
        fs.renameSync(
          ...
          path.join(projectDir, submodule, '.git.andjibackup'),
        )
...
function restoreSubmodules({ projectDir }: { projectDir: string }) {
  for (const nestedRepo of nestedRepos) {
    ...
    try {
      fs.renameSync(andjiBackup, gitDir)
    } catch (error) {
      console.error(
        `Failed to restore .git directory for nested repo. Please rename ${andjiBackup} to ${gitDir}\n${JSON.stringify({ error }, null, 2)}`,
      )
      logger.error(
        {
          error,
