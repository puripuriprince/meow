...
import { isSubdir } from '@andji/common/util/file'
import {
  stripColors,
  truncateStringWithMessage,
} from '@andji/common/util/string'
...
function createWrapperScript(
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'andji-')) // safe unique dir
...
export const runTerminalCommand = async (
  ...
  const resolveCommand = (
    value: andjiToolOutput<'run_terminal_command'>,
  ) => {
    commandIsRunning = false
...
const runCommandChildProcess = async (
  ...
  resolve: (value: andjiToolOutput<'run_terminal_command'>) => void,
) => {
  const projectRoot = getProjectRoot()
