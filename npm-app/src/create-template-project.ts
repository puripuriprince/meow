...
export async function createTemplateProject(
  ...
  try {
    ...
    const tempDir = fs.mkdtempSync(join(os.tmpdir(), 'andji-starter-'))
    execSync(
      'git clone --depth 1 https://github.com/andjiAI/andji-community.git .',
      {
        cwd: tempDir,
        stdio: 'pipe',
      },
    ...
    // Change into the new project directory and run andji
    process.chdir(projectPath)
    console.log('Starting andji in the new project...\n')
    console.log('--------------------------------\n')
    execSync('andji', { stdio: 'inherit' })
  } catch (error) {
    logger.error(
      {
